...
namespace Kerbalua.Gui {
    public class KerbaluaRepl {
        ...
        public void Render(bool guiActive){
            ...
            try {
                windowRect = GUI.Window(windowID, windowRect, ReplWindow, "kerbalua REPL");
                CompletionBox();
                inputChanged = false;
            } catch (Exception e) {
                Debug.Log(e);
        ...
        Rect outputScrollRect= new Rect(0, 0, 700, 400);
        ...
        void ReplWindow(int id)
        {
            ...
            HandleInput();
            InputOutputBox(windowRect);
        }

        void InputOutputBox(Rect parentRect)
        {
            float inputHeight = outputStyle.CalcHeight(inputContent, parentRect.width);
            float inputStart = parentRect.height - inputHeight;

            Rect outputRect = new Rect(0, 0, parentRect.width, inputStart);
            Rect inputRect = new Rect(0, inputStart, parentRect.width, inputHeight);
        ...
        void HandleInput(){
            ...
            if (Mouse.Left.GetClick() || Mouse.Right.GetClick()) {
                if (windowRect.Contains(new Vector2(e.mousePosition.x + 100, e.mousePosition.y + 100))) {
