...
namespace Kerbalua.Events
{
	...
	/// </summary>
	public class LuaEventDescriptor : IMemberDescriptor
	{
		/// <summary>
		...
		protected MultiValueDictionary<object, Closure> callbacks = new MultiValueDictionary<object, Closure>(new ReferenceEqualityComparer());
		protected Dictionary<object, Delegate> delegates = new Dictionary<object, Delegate>(new ReferenceEqualityComparer());
		...
		public static LuaEventDescriptor TryCreateIfVisible(EventInfo ei, InteropAccessMode accessMode)
		{
			...
			if (ei.GetVisibilityFromAttributes() ?? ((remm != null && remm.IsPublic) && (addm != null && addm.IsPublic)))
				return new LuaEventDescriptor(ei, accessMode);
		...
		public LuaEventDescriptor(EventInfo ei, InteropAccessMode accessMode = InteropAccessMode.Default)
		{
		...
		public DynValue GetValue(Script script, object obj)
		{
			...
			return UserData.Create(new LuaEventProxy(this, obj));
		}
		...
		internal DynValue RemoveCallback(object o, ScriptExecutionContext context, CallbackArguments args)
		{
			lock (sync)
			{
				...
				if (callbacks.RemoveValue(o, closure))
		...
		private Delegate CreateDelegate(object sender)
		{
			...
			default:
				throw new ScriptRuntimeException("too many args in delegate type");
			}
		}
			...
			object o13 = null, object o14 = null, object o15 = null, object o16 = null)
		{
			...
			lock (sync)
			{
				closures = callbacks.Find(sender).ToArray();
			}
