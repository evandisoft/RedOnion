...
def gui
  ...
  var boxWidth = 60
  var uniWidth = 40
  var wnd = new window "Launch " + ship.name
...
if !evan
  ...
  if cust
    def steerPID
      var pid = PID 1.0, 0.5, 0.01, 0.4
      pid.maxInput = 1
      pid.minInput = -1
      pid.maxOutput = 1
      pid.minOutput = -1
      pid.outputChangeLimit = 5
      pid.accumulatorLimit = 0.5
      pid.errorLimit = 1
      return pid
    var ppid = steerPID()
    var ypid = steerPID()
...
//smooth transition from surface to orbital speed by ratio of altitude to safe altitude
  ...
  if altitude <= firstAltitude
    want = startAngle - (startAngle - firstAngle) * altitude/firstAltitude
  else
...
for
  ...
  if eta <= 5 and !switched
  //TODO: warp to this point
    switched = true
    ctlsub.remove
