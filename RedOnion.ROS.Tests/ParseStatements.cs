...
{
	[TestFixture]
	public class ROS_ParseStatements : ParserTests
	{
		public void Test(string source, Action prefix, Action postfix)
			=> Test(source, null, prefix, postfix);
		...
		public void ROS_PStts01_Return()
		{
			...
			() =>
			{
				...
				CodeCheck(1, OpCode.Void);
				CodeCheck(2);
			},
			() =>
			{
				CodeCheck(0, OpCode.Void);
				CodeCheck(1, OpCode.Return);
				CodeCheck(2);
			});
		}
		...
		public void ROS_PStts02_ReturnExpression()
		{
			...
			() =>
			{
				...
				CodeCheck( 1, 0, "a");
				CodeCheck( 5, OpCode.LogicAnd);
				CodeCheck( 6, 5); // size of second argument for short-circuit
				CodeCheck(10, OpCode.Identifier);
				CodeCheck(11, 1, "b");
				CodeCheck(15, OpCode.Return);
				CodeCheck(16);
			});
		}
		...
		public void ROS_PStts09_TryCatchFinally()
		{
			...
			CodeCheck(19, OpCode.Void); // any exception
		...
		public void ROS_PStts10_SwitchCase()
		{
			...
			CodeCheck(30, OpCode.Void);
			CodeCheck(31, OpCode.Void); // default
		...
		public void ROS_PStts11_Lambda()
		{
			...
			CodeCheck(19, OpCode.Void);
			CodeCheck(20, 0, "e");
			CodeCheck(24, 1);
			CodeCheck(28, OpCode.Void); // no type
			CodeCheck(29, 1);
			CodeCheck(33, OpCode.Void); // no default
